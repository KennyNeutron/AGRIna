...
import { DeviceSelector } from "@/app/(dashboard)/live-readings/device-selector";
...
export default async function HomePage(props: {
  ...
  // Fetch Stats
  const { count: devicesCount, data: devices } = await supabase
    .from("devices")
    .select("*", { count: "exact" });
  ...
  const selectedDeviceId = searchParams.device || devices?.[0]?.id;
  ...
  // Safe defaults
  const deviceName = latestReading?.devices?.name || "No Device Selected";
