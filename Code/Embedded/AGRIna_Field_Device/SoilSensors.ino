...
struct NpkLock {
  bool ok = false;
  uint8_t fc = 0x03;
  uint16_t base = 0x001E;
  bool byScan = false;
} npk;

static inline bool allZero(uint16_t a, uint16_t b, uint16_t c) {
  return (a == 0 && b == 0 && c == 0);
}
  ...
  for (uint8_t i = 0; i < sizeof(tries) / sizeof(tries[0]); i++) {
    if (mbReadRegsFC(RS485_A, A_REDE_PIN, NPK_ID, tries[i].fc, tries[i].base, 3, r)) {
      if (plausible(r[0], r[1], r[2])) {
        npk.ok = true;
        npk.fc = tries[i].fc;
        npk.base = tries[i].base;
        npk.byScan = true;
        N = r[0];
        P = r[1];
        K = r[2];
        Serial.print(F("[NPK] Locked by scan: FC0x"));
        Serial.print(npk.fc, HEX);
        Serial.print(F(" @ 0x"));
        Serial.println(npk.base, HEX);
        return true;
      }
    }
  }
