...
void loop() {
  ...
  if (idx != (int)sizeof(AGRInaData)) {
    Serial.print(F("[WARN] Unexpected size. Got "));
    Serial.print(idx);
  ...
  if (!ok) {
    ...
    Serial.print(F("  Footer=0x"));
    Serial.println(rx.Footer, HEX);
    hexDump(buf, idx);
    return;
  }
  ...
  Serial.println(rx.AN_DeviceID);
  Serial.print(F("Soil Temp (°C): "));
  Serial.println(rx.AN_SoilTemperature, 2);
  Serial.print(F("pH: "));
  Serial.println(rx.AN_pH, 2);
  Serial.print(F("Nitrogen: "));
  Serial.println(rx.AN_Nitrogen);
